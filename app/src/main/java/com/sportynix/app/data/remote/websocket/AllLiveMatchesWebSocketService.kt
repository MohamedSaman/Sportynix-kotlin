...
sealed interface LiveMatchWSEvent {
    data class MatchSnapshotUpdate(val matches: List<LiveMatchSnapshot>) : LiveMatchWSEvent
    data class SingleMatchScoreUpdate(val matchId: String, val liveData: LiveMatchSnapshot) : LiveMatchWSEvent
}

@Singleton
class AllLiveMatchesWebSocketService @Inject constructor(
    ...
    private val _events = MutableSharedFlow<LiveMatchWSEvent>(extraBufferCapacity = 64)
    val events: SharedFlow<LiveMatchWSEvent> = _events.asSharedFlow()
    ...
    fun connect() {
        ...
        webSocket = okHttpClient.newWebSocket(request, object : WebSocketListener() {
            ...
            override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
                ...
                this@AllLiveMatchesWebSocketService.webSocket = null
                scheduleReconnect()
            }
    ...
    private fun sendFilter(status: String) {
        ...
        scope.launch {
            try {
                ...
                socket.send(gson.toJson(filterMsg))
            } catch (e: Exception) {
                Timber.e(e, "Error sending filter")
            }
        }
    }
        ...
        try {
            ...
            when (type) {
                "matches_snapshot", "matches_page" -> {
                    ...
                    scope.launch {
                        _events.emit(LiveMatchWSEvent.MatchSnapshotUpdate(matchesList))
                    }
                }
                "live_match_added", "live_match_updated", "match_update", "match_live_score_update" -> {
                    ...
                    if (updatedMatch != null) {
                        scope.launch {
                            _events.emit(LiveMatchWSEvent.SingleMatchScoreUpdate(updatedMatch.matchId, updatedMatch))
                        }
                    }
                }
                "match_completed" -> {
                    ...
                    if (completedMatch != null) {
                        val finalMatch = completedMatch.copy(status = "completed")
                        scope.launch {
                            _events.emit(LiveMatchWSEvent.SingleMatchScoreUpdate(finalMatch.matchId, finalMatch))
                        }
                    }
                }
            }
